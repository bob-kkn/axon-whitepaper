...
| Issuance authorization not bound to an amount | If the adjudication result reaches the ledger as an opaque token, the ledger cannot verify the issued amount and an inflated issuance passes | Design the authorization as a verifiable proof bound to the amount as well (fixed at the implementation stage) |
