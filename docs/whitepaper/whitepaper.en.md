...
| Issuance authorization not bound to an amount (mitigated) | If the adjudication result reaches the ledger as an opaque token, the ledger cannot verify the issued amount and an inflated issuance passes | Converted to a verifier-signed attestation bound to the amount and issuer — implemented and verified in a Phase 1 prototype (inflated issuance 158/200 → 0/200). Residual: single-key verifier trust; a k-of-N committee is an implementation-stage item |
