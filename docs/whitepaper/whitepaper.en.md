...
**What remains is the authority itself, and the prototype publishes its size in numbers.** Because it chooses the parents knowing the seed, the authority can still seat a colluder registered beforehand (two of three seats), or conversely parent an opening at genesis and exclude every honest registrant — censorship. Censorship is not even the authority's alone: with no sender authentication, a third party can invalidate the transaction an opening hangs from with a single transfer and erase the epoch, after which recovery needs the authority's key. A heavier fact was also measured: single-use for issuance authorizations is per (epoch, supplier) rather than per epoch, and supplier is unauthenticated, so **a committee that captures one epoch can mint without limit, and indefinitely, since epochs do not expire** — twenty issuances multiplied the supply two-hundredfold while all eleven invariants held. The hypergeometric table therefore prices the **probability of capture, not the loss given capture.** A per-epoch issuance cap with expiry, and moving the beacon on-ledger so the authority cannot choose the seed, are the next slices. That the registry's authenticity is unauthenticated, and that the ledger cannot check whether a signed amount equals the work actually verified, both remain.
